...
- 正式的整合輸出是XeLaTeX建置的261頁A4教學文件（2026-08-22重整教學文件前為271頁：使用與編寫教學提前，起因與老師們的話移入附錄，安裝截圖章改為Overleaf／終端機／編輯器三種方式）。輸出敏感的修改除了比對頁數與文字，還要比對正規化的文字座標、字型表、固定DPI點陣圖，並人工檢視受影響的頁面。
