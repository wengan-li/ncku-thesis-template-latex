...
- Overleaf Gallery是獲批准的公開編輯入口，GitHub Releases是版本化下載的正式來源；Gallery套件會清除Draft標記與學校浮水印資產，且`just test`會冷建置產生的Gallery套件防止契約漂移。Gallery目前載有2026-07-18上傳的v2.0.0時期版本；自2026-08-22起，凡更動`thesis/template/**`、`thesis/thesis.tex`或學生建置路徑的發行，都要重新產生Gallery套件交owner更新原Overleaf專案，純文件發行不需要。
