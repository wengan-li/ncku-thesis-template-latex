...
- 發行後仍要重新下載公開資產，核對SHA-256並與workflow artifact逐位元比對；v2.0.2、v2.0.5、v2.0.6、v2.0.7與v2.0.8各次發行的read-back證據記錄在[驗證與效能](validation-and-performance.md)。
- Overleaf Gallery是獲批准的公開編輯入口，GitHub Releases是版本化下載的正式來源；Gallery套件會清除Draft標記與學校浮水印資產，且`just test`會冷建置產生的Gallery套件防止契約漂移。Gallery於2026-08-22以`v2.0.7.260822120950`來源更新（之前為2026-07-18上傳的v2.0.0時期版本），公開PDF於2026-08-23讀回核對，文字與本地同一套件的建置完全相同；Overleaf的發佈表單只提供CC BY 4.0一個選項，因此Gallery上的copy由owner以CC BY 4.0提供，GitHub repository與兩個發行套件維持CC BY-NC-SA 4.0；這是owner於2026-08-23有意的雙授權決定，不是待修正的差異。自2026-08-22起，凡更動`thesis/template/**`、`thesis/thesis.tex`或學生建置路徑的發行，都要重新產生Gallery套件交owner更新原Overleaf專案，純文件發行不需要。
