...
- Complete theorem hardening fixture and first registry slice: all 21 public insertion/setter routes compile through one focused runtime contract covering default format metadata, numbered and unnumbered forms, optional titles, 15 labels, `\ref`/`\nameref`, section reset, the proof marker, and unknown/empty no-op behavior. The fixture protects the corrected optional-label and mutable-title metadata defects; one private ordered type list now drives aggregate initialization and setter dispatch while per-type wrappers, key families, styles, and counters remain unchanged.
...
The first registry slice is complete: one private ordered type registry drives aggregate initialization and `\SetTheoremFormat` dispatch, every registered setter route plus unknown/empty no-op behavior is covered, and every per-type public wrapper, initializer, key family, theorem style, and counter implementation remains intact. A later independently approved slice may consider generating declarations or counter logic, but only with additional style/counter fixtures.
