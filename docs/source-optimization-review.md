...
The source API scanner now strips LaTeX `comment` environments before collecting declarations. The corrected baseline contains 597 runtime-visible v1 names, while `tests/v1-comment-environment-artifacts.json` preserves an explicit audit of 22 dead declarations: 15 comment-only names plus 7 overlaps with live tombstones, including one false extra signature. The three numbering comment blocks and obsolete bibliography block were removed only after this boundary was established. Full registry generation remains deferred; the numbering correction and dead-code cleanup are compatibility-bounded and fixture-backed.
