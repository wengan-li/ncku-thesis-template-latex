...
- Complete theorem hardening, first registry, and numbered-initialization slices: all 21 public insertion/setter routes compile through focused runtime contracts covering default/custom metadata, numbered and unnumbered forms, optional titles, 15 labels, `\ref`/`\nameref`, section reset, the proof marker, unknown/empty no-op behavior, and plain-versus-definition styles. The custom matrix protects global, Section, and chained-to-unscoped parent counters; parent resolution now happens before the scoped/unscoped `\newtheorem` choice. One private ordered type list drives aggregate initialization/setter dispatch, and one private numbered initializer removes the repeated branch while all per-type public wrappers, key families, styles, and counters remain available.
...
- Deferred to later major-version experiments: numbering registry rewrites, deeper theorem declaration/counter generation, class/package redesign, broad `expl3` conversion, `l3build`, engine migration, and tagged-PDF/PDF-UA work. The first theorem registry slice is complete and intentionally stops at ordered initialization/setter dispatch.
...
The first registry and numbered-initialization slices are complete: one private ordered type registry drives aggregate initialization and `\SetTheoremFormat` dispatch, and one private numbered initializer resolves a referenced type's effective parent before choosing the scoped or unscoped `\newtheorem` form. Every registered setter route plus unknown/empty no-op behavior is covered; the style/counter matrix covers all 21 custom environment/text routes, plain-versus-definition body style, and global/Section/chained-empty counters. Every per-type public wrapper, initializer, key family, theorem style, and counter getter remains available. A later independently approved slice may consider generating the repeated key-family declarations, but only with equivalent custom-environment evidence.
