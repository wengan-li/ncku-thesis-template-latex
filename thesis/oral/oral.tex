
% ----------------------------------------------------------------------------
%                           Document of oral presentation
%                                   口試証明文件
% ----------------------------------------------------------------------------

% ----------------------------------------------------------------------------

\ifthenelse{\OralDocument = \OralDocumentTemplete}
{
  % -------------------------- 顯示範本 --------------------------
  \ifthenelse{\LangEnableChi = 1}
  {
    \StartNewPage

    % 設定顯示在目錄
    % If combine with english version
    \ifthenelse{\GetLangEnableEng = 1}
    {
      \addcontentsline{toc}{chapter}{Document of oral presentation}
    }{}

    % Chinese version
    \input{./oral/templete/templete-chi.tex}

    % ------------------------------------------------
    \ifthenelse{\GetLangEnableDemo = 1}
    {
      \input{./oral/templete/templete-chi-2.tex}
      \input{./oral/templete/templete-chi-3.tex}
      \input{./oral/templete/templete-chi-4.tex}
    }{}
    % ------------------------------------------------
  }{}

  % English page start
  \ifthenelse{\GetLangEnableEng = 1}
  {
    \StartNewPage

    % 設定顯示在目錄
    % If combine with chinese version
    \ifthenelse{\LangEnableChi = 1}
    {
      \addcontentsline{toc}{section}{English version}
    } % End of if{}
    {
      \addcontentsline{toc}{chapter}{Document of oral presentation}
    } % End of else{}

    \thispagestyle{empty}
    \input{./oral/templete/templete-eng.tex}
  }{}
} % End of if{}
{
  % ---------------------- 顯示圖片, Image in PDF format ----------------------

  % Page start
  \StartNewPage
  \thispagestyle{empty}

  % 設定顯示在目錄
  \addcontentsline{toc}{chapter}{Document of oral presentation}

  % Chinese page start
  \ifthenelse{\LangEnableChi = 1}
  {
    \addcontentsline{toc}{section}{Chinese version}
    \includepdf{\GetOralImageChiPath}
  }{}

  % English page start

  % If combine with chinese version
  \ifthenelse{\LangEnableChi = 1}
  {
    \StartNewPage
    \thispagestyle{empty}
    \addcontentsline{toc}{section}{English version}
  }{}

  \includepdf{\GetOralImageEngPath}
} % End of else{}

% ----------------------------------------------------------------------------

% End of page
\EndOfPage

% ----------------------------------------------------------------------------
