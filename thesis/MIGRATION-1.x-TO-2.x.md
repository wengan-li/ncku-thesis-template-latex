...
| A numbered theorem configured to follow another theorem whose effective parent counter was empty selected the scoped branch before resolving that parent, then failed with `No counter '' defined`. The `Theorem` initializer also hard-coded `section` after mapping instead of using the resolved parent. | Numbered formats resolve the configured parent first, then select scoped or unscoped `\newtheorem` syntax from the effective value. Global, Section, and chained-to-unscoped custom counters now compile consistently. | None; custom theorem-format users should rebuild. |
