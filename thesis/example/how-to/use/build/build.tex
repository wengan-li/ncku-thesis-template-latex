...
注意: 本節的截圖與選單路徑來自MiKTeX 2.9與舊版Texmaker的年代, 現行版本的畫面會不同 (新版MiKTeX改用MiKTeX Console更新package). 其實只要發行版包含XeLaTeX, BibTeX與\texttt{latexmk}即可, MiKTeX, TeX Live或MacTeX都可以使用; 編輯器亦不限於Texmaker.
...
並以`thesis.tex'作為主文件。\texttt{latexmk}會按需要自動執行XeLaTeX、BibTeX及重複編譯，不需要人手猜測次序。圖\RefTo{fig:how-to:use:build:texmaker:to_xelatex}所示的直接XeLaTeX模式仍可作基本相容用法，但正式文件應以\texttt{latexmk}完成所有引用及目錄收斂。(註: 第一次使用時MiKTeX可能會自動下載工具，所以會等待比較久。)
