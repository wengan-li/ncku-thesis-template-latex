...
建議將Texmaker的編譯命令設為\verb|latexmk -xelatex -synctex=1 -interaction=nonstopmode %.tex|，並以`thesis.tex'作為主文件。\texttt{latexmk}會按需要自動執行XeLaTeX、BibTeX及重複編譯，不需要人手猜測次序。圖\RefTo{fig:how-to:use:build:texmaker:to_xelatex}所示的直接XeLaTeX模式仍可作基本相容用法，但正式文件應以\texttt{latexmk}完成所有引用及目錄收斂。(註: 第一次使用時MiKTeX可能會自動下載工具，所以會等待比較久。)
...
在編譯LaTeX成PDF時，必須讓目錄、交叉引用及參考文獻(例如\verb|\RefBib{}|)完成所需的多輪處理。請在專案目錄執行：

\begin{center}
\verb|latexmk -xelatex -synctex=1 -interaction=nonstopmode thesis.tex|
\end{center}

\texttt{latexmk}會讀取dependency及auxiliary files，只執行當次需要的XeLaTeX及BibTeX passes；一般正文修改通常不需要重新執行BibTeX。

寫作時可以保持continuous preview：

\begin{center}
\verb|latexmk -xelatex -pvc -view=none -synctex=1 thesis.tex|
\end{center}

儲存任何被引用的TeX、BibTeX或圖片檔後，PDF會自動重建。\verb|-view=none|避免另外打開第二個viewer；可繼續使用Texmaker或TeXstudio內置、能自動重新載入的PDF viewer。停止監察可按\texttt{Ctrl-C}。

實際編寫自己論文時，請在\verb|conf/conf.tex|關閉\verb|\ExampleMode|，讓主文件使用\verb|context/context.tex|；完整教學範例包含大量圖片及附錄PDF，重建時間會較長。
