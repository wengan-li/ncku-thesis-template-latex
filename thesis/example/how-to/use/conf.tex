...
\begin{enumerate}
  ...
  \item
  {
    ...
    寫作或審閱期間可使用`\verb|\DisplayDraft|'，在中文版／英文版封面顯示`(初稿)'／`(Draft)'。這個封面標記與斜向的`DRAFT'文字浮水印是兩個獨立功能；模版預設同時關閉。正式輸出及上傳學校系統前應保持關閉。

    學校logo浮水印亦預設關閉。只有當時的正式要求明確需要時，才使用`\verb|\UseWatermarkFigureStyle|'；不要因為模版提供此API便自行加入提交檔案。
  } % End of \item{}
  ...
  \item
  {
    ...
    正式提交應優先使用學位考試系統產出的證明書檔案，並以學校當時的最新規定為準。模版產生的範例只作legacy/example用途，不代表學校正式文件.

    如果要用的是範例:\\
    \verb|\DisplayOralTemplate|: 顯示 / 使用 口試範例版本.\\
    \verb|\DisplayOralChiTemplate|: 顯示中文範例版本\\
    \verb|\DisplayOralEngTemplate|: 顯示英文範例版本\\
    \verb|\SetCommitteeSize{9}|: 口試委員數量, 要配合\verb|\DisplayOralTemplate|來使用；請先選擇學位。NCKU碩士為3至5位，博士為5至9位，超出範圍會收斂到最近的有效值，預設為9位.\\
...
由abbrv/plain轉去apacite必定需要刪除檔案才能進行.
  ...
  \item
  {
    \textbf{插入DOI碼 (Legacy/Custom)}

    \verb|\ShowDOI|:  插入DOI碼\\
    現行ETDS上傳流程不需要自行加入DOI、浮水印或保全設定. 只有其他明確要求時才使用這個API，並應先確認學校的最新上傳指引.
  } % End of \item{}
