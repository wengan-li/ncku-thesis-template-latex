...
\StartSubSection{單張}
  ...
  \item
  {
    把圖放在表格中, 這時候是使用\verb|\InsertFigure{}|, 是不能使用caption和label (正常寫法應該用不到, 例如出現'Fig X.X'這種字在table中). (有關表格table的使用, 請參考Chap \RefTo{chapter:how-to:write:table}). 如真的想使用, 則考慮這邊的寫法.
...
\StartSubSection{多張}
  ...
  \item
  {
    ...
    [align = center,
      caption = {2 images and 1 image per row}] %
    {
      [align = center]{./example/how-to/write/image/pic/CC-BY-NC.png}
    }%
    {
