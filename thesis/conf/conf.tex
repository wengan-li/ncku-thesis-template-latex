...
% --- DOI 碼 ---
% 現行ETDS上傳流程不需要自行加入DOI、浮水印或保全設定.
% 只有在其他明確要求下才啟用這個legacy/custom API.
%\ShowDOI{doi:10.6844/ncku.latex.template.2023.Z00}
...
% --- Draft 初稿 ---
% 寫作／審閱階段如需在封面顯示 '(初稿)' 或 '(Draft)'，才取消下一行註解.
% 正式輸出及上傳學校系統前應保持關閉.
%\DisplayDraft

% 如另需斜向的文字浮水印，可明確設定文字；模版不會使用draftwatermark
% package的預設'DRAFT'字樣.
%\SetWatermarkText{DRAFT}

% --- Institutional watermark 學校浮水印 ---
% 本模版預設不加入學校logo浮水印；學校系統會按當時流程處理正式電子全文.
% 只有其他明確要求或本地測試時，才取消下一行註解.
...
% Legacy範例版本只供示範；正式提交應使用學位考試系統產出的檔案.
%\DisplayOralTemplate
...
\DisplayOralImage                 % 顯示學位考試系統產出的圖檔
