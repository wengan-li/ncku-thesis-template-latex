...
% Some base packages
\usepackage{geometry}
% Public helpers retain xparse-only G arguments throughout the v2 compatibility
% window; declare that dependency instead of relying on fontspec to load it.
\usepackage{xparse}
\usepackage{ifthen}
\usepackage{amsmath}
...
% 論文有關資料
% Normal student builds keep the historical path. Repository profile fixtures may
% select an isolated generic configuration without modifying the byte-pinned V1
% migration input.
\providecommand{\TemplateConfigurationFile}{./conf/conf}
...
% Initialize all theorem formats
\InitTheoremFormats
