...
{%
  ...
  % The project does not bundle a separate CJK monospaced face. Keep CJK text
  % inside \texttt on the selected CJK family instead of xeCJK's undefined
  % \CJKttdefault fallback; Latin monospaced text remains unchanged.
  \setCJKmonofont[%
    Path = \GetFontDirPath,
    UprightFont = \TmpValueNormalFont,
    AutoFakeBold = true,
...
\def \InitFontStyleNotoSansCJK
{
  ...
  \SetChiMainFont[%
    NormalFont = \VarFontTypeNotoSansCJKChiFileNameNormal,%
...
% \if compares one token at a time, so the two-digit \VarFontTypeCustom value
% can never match itself there; \ifnum compares the complete numeric values.
\newcommand{\UseDefaultFontType}
{%
  \ifnum \GetFontUseType=\VarFontTypeTimesKaiu\relax%
    \UseFontStyleTimesKaiu%
  \fi%
%
  \ifnum \GetFontUseType=\VarFontTypeNotoSansCJK\relax%
    \UseFontStyleNotoSansCJK%
  \fi%

  \ifnum \GetFontUseType=\VarFontTypeCustom\relax%
    \UseFontStyleCustom%
  \fi%
} % End of \newcommand{}
