...
% The active institution profile owns any oral-template watermark decision;
% these generic defaults apply none. See template/style/ncku/ncku.tex for the
% NCKU legacy-certificate policy.
\newcommand{\ApplyOralTemplateChiWatermarkPolicy}{}
\newcommand{\ApplyOralTemplateEngWatermarkPolicy}{}

\newcommand{\StartOralTemplateDocChi}
{
  % 先清掉現有watermark, 再由profile policy決定這頁的樣式
  \ClearWatermarkStyle
  %
  \ApplyOralTemplateChiWatermarkPolicy
  %
  \singlespacing%
  %
  \StartNewPage
...
\newcommand{\StartOralTemplateDocEng}
{
  ...
  \ClearWatermarkStyle
  %
  \ApplyOralTemplateEngWatermarkPolicy
  %
  \singlespacing%
  %
  \StartNewPage
...
% Generic renderer capacity is 2--9. The active institution profile may
% override this hook with degree-specific policy without changing the public
% \SetCommitteeSize signature.
\newcommand{\ApplyCommitteeSizePolicy}[1]
{
  \ifthenelse{#1 < 2}
  {
...
\newcommand{\SetCommitteeSize}[1]{\ApplyCommitteeSizePolicy{#1}}
...
\newcommand{\FlagOralDocumentType}{\ValueOralDocumentTypeImage} % Default
